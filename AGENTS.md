...
this a RPG game like Dragon quest in java from scratch widout using any game engine.
...
- This file is called **"AGENT.md"** and it is inside the root of the project.
...
- if i tell u to check the error u can run `make run`, run just one time, if you fix the problem and run `make run` the game start and you can't pay it sorry

## Project structure
- `src/`: main java sources. Organized by package prefixes to keep responsibilities focused.
  - `dev.typegaro.drimu.core`: window bootstrap, game loop timing, and user input glue (`Main`, `GamePanel`, `InputHandler`).
  - `dev.typegaro.drimu.entity`: base actor hierarchy (`Entity`, `EntityInterface`) plus concrete actors like `Player`. Anything that moves or animates in the world belongs here.
  - `dev.typegaro.drimu.graphics`: sprite loading/animation helpers (`SpriteManager`, `SpriteManagerInterface`).
  - `dev.typegaro.drimu.geometry`: math helpers such as `Vector2D`.
- `res/`: sprite sheets and other runtime resources referenced by the graphics package.
- `out/`: build artifacts created by the IDE/tooling; do not hand-edit.

## Files usually ignored
- `flake.nix`, `flake.lock`, `devenv.nix`: project Nix/devenv definitions. Touch only if the development environment itself needs tweaks.
- `out/`: generated bytecode/resources; safe to ignore unless debugging the build.
